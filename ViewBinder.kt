...
class ViewBinder(private val viewFinder: Finder) {
    ...
    operator fun <V : View> invoke(@IdRes id: Int, viewFinder: Finder = this.viewFinder, viewInitializer: ViewInitializer<V>? = null) =
            required(id, viewFinder, viewInitializer).register()
    operator fun <V : View> invoke(ids: IntArray, viewFinder: Finder = this.viewFinder, viewInitializer: ViewInitializer<List<V>>? = null) =
            required(ids, viewFinder, viewInitializer).register()

    fun <V : View> optional(@IdRes id: Int, viewFinder: Finder = this.viewFinder, viewInitializer: ViewInitializer<V?>? = null) =
            nullable(id, viewFinder, viewInitializer).register()
    fun <V : View> optional(ids: IntArray, viewFinder: Finder = this.viewFinder, viewInitializer: ViewInitializer<List<V?>>? = null) =
            nullable(ids, viewFinder, viewInitializer).register()

    fun <V : View> stub(@IdRes id: Int, viewFinder: Finder = this.viewFinder, viewInitializer: ViewInitializer<V>? = null): ReadOnlyProperty<Any, V> = stubRequired(id, viewFinder, viewInitializer)
    fun <V : View> optionalStub(@IdRes id: Int, viewFinder: Finder = this.viewFinder, viewInitializer: ViewInitializer<V?>? = null): ReadOnlyProperty<Any, V?> = stubNullable(id, viewFinder, viewInitializer)

    fun <T : Any> stuff(initializer: () -> T): ReadOnlyProperty<Any, T> = Lazy { initializer() }.register()
